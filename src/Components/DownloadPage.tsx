...
const DownloadPage: React.FC<DownloadPageProps> = ({ selectedExerciseProp }) => {
    ...
    const onDownload = async () => {
        ...
        pdf.setFontSize(12);
        pdf.setTextColor(60);
        pdf.text(`Exercise ID: ${id}`, 10, 35);
        pdf.text(`Wallet address: ${address}`, 10, 45);

        // Description
        pdf.setFontSize(11);
        const description = `This is the download receipt for your exercise "${name}". Bring it to the center to access the corresponding courses.`;
        const splitText = pdf.splitTextToSize(description, 180);
        pdf.text(splitText, 10, 60);

        // Footer (optional)
        pdf.setFontSize(10);
        pdf.setTextColor(150);
        pdf.text('Generated by Yoyo Marketplace', 10, 280);

        // Save the PDF
        pdf.save(`${name} receipt.pdf`);
    };
